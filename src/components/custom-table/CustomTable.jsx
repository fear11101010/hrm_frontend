...
function CustomTable({
  ...
  responsive,
  onDataSort,
  pagination: { show, perPageList, onPageOrLimitChange },
}) {
  ...
  const afterDataSort = (d) => {
    setTableRows(
      show && !total ? d?.slice((pageNumber - 1) * (showPerPage?.value || 0), pageNumber * (showPerPage?.value || 0)) : d
    );
    setDummy((t) => -t);
  };
      ...
      <div className="card-header">
        <div className="row align-items-center">
          ...
          <div className="col-auto me-n3">
            <Form>{show && <Select options={paginationOption} onChange={onPageLimitChange} value={showPerPage} />}</Form>
          </div>
        </div>
      </div>
          ...
          {tableRows && Array.isArray(tableRows) && tableRows.length > 0 ? (
                      ...
                      <td style={{width:column?.width?`${column.width}px`:''}}>
                        {column.cell ? (
                          ...
                          <span className="item-title">
                            {column.selector(v, (pageNumber - 1) * (showPerPage?.value || 0) + i)}
                          </span>
                        )}
                      </td>
                    ))}
            ...
            <tbody>
              <tr key="no_data_available_row">
                <td key="no_data_available" colSpan={columns.length}>
                  No Data Available
                </td>
              </tr>
            </tbody>
