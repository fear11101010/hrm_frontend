...
function ExcelPdfPrint({exportExcel, exportPdf, print, data, columns, header}) {
    ...
    const generatePdf = (e) => {
        ...
        tempImg.src = 'data:image/svg+xml,' + encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}">
...
${ReactDOM.findDOMNode(element).outerHTML.replace("display: none;", '')}
    ...
    return (
        <>
            <div className="mb-2">
                {(data && Array.isArray(data) && data.length > 0) && (<ButtonGroup>
                    {exportExcel && <Button variant="primary" size="sm" onClick={generateExcel}>
                        <FaFileExcel/> Excel
                    </Button>}
                    {exportPdf && <Button variant="primary" size="sm" onClick={generatePdf}>
                        <FaFilePdf/> Pdf
                    </Button>}
                    {print && <Button variant="primary" size="sm" onClick={printData}>
                        <FaPrint/> Print
                    </Button>}
                </ButtonGroup>)}
            </div>
                ...
                <div  ref={tableRef} style={{display: 'none', fontSize: '20px',}}>
                    ...
                    <table width='100%' border="1"
                        ...
                        <thead>
                            {tableColumns && (
                                <tr>
                                    {tableColumns.map((v,i) => (<th key={`${v}_${i}`} style={{
                                        wordBreak: 'normal',
                                        wordWrap: 'normal',
                                        border: '1px solid'
                                    }}>{v}</th>))}
