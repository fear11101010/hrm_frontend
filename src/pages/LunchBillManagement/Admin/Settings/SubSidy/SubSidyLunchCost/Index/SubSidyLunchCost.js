...
function SubSidyLunchCost(props) {
    ...
    const updateSubSidy = (data, e) => {
        setLoading(true)
        API.put(SUBSIDY_UPDATE_DELETE_API(data.id),data).then(success => {
    ...
    const createSubSidy = (data, e) => {
        setLoading(true)
        API.post(SUBSIDY_LIST_CREATE_API,data).then(success => {
            success_alert(success?.data?.message)
            setRefresh(!refresh)
        }).catch(error => {
            error_alert(error.data.message)
    ...
    const deleteSubSidy = (e) => {
        setLoading(true);
        API.delete(SUBSIDY_UPDATE_DELETE_API(data?.data[getValues('index')]?.id)).then(success => {
            success_alert(success?.data?.message)
            setRefresh(!refresh)
        }).catch(error => {
            error_alert(error.data.message)
    ...
    return (
        <Layout>
            ...
            {(showConfirmDialog) && <ConfirmDialog message="Are you sure delete subsidy?" onOkButtonClick={deleteSubSidy} onCancelButtonClick={(e)=>setShowConfirmDialog(false)}/>}
        </Layout>
    );
