...
export default function LunchOrderTimeConfiguration(props) {
    ...
    const am_pm = [{value: 'AM', label: 'AM'}, {value: 'PM', label: 'PM'}]
    ...
    const saveData = (e) => {
        setLoading(true);
        const formData = [
            {...lunchOrder}, {...menuEntry}, {...orderCancel}
        ]
        let method = 'post';
        ...
        API[method](url, formData).then(success => {
                    ...
                    <Card.Body>
                        <Row>
                            ...
                            <Col sm={12} md={7} lg={7}>
                                <Row>
                                    <Col className="mb-2">
                                        <Select options={hours} placeholder="Hour"
                                                onChange={(v) => changeOrder(v, 'hour')} value={hours.find(h=>h.value===lunchOrder?.hour)}>

                                        </Select>
                                    </Col>
                                    <Col className="mb-2">
                                        <Select options={minutes} placeholder="Minute"
                                                onChange={(v) => changeOrder(v, 'minute')} value={minutes.find(h=>h.value===lunchOrder?.minute)}>

                                        </Select>
                                    </Col>
                                    <Col className="mb-2">
                                        <Select options={am_pm} onChange={(v) => changeOrder(v, 'meridiem')}
                                                placeholder={"AM or PM"}  value={am_pm.find(h=>h.value===lunchOrder?.meridiem)}>
                        ...
                        <Row>
                            ...
                            <Col sm={12} md={7} lg={7}>
                                <Row>
                                    <Col className="mb-2">
                                        <Select options={hours} placeholder="Hour"
                                                onChange={(v) => changeMenuEntry(v, 'hour')} value={hours.find(h=>h.value===menuEntry?.hour)}>

                                        </Select>
                                    </Col>
                                    <Col className="mb-2">
                                        <Select options={minutes} placeholder="Minute"
                                                onChange={(v) => changeMenuEntry(v, 'minute')} value={minutes.find(h=>h.value===menuEntry?.minute)}>

                                        </Select>
                                    </Col>
                                    <Col className="mb-2">
                                        <Select options={am_pm} onChange={(v) => changeMenuEntry(v, 'meridiem')}
                                                placeholder={"AM or PM"}  value={am_pm.find(h=>h.value===menuEntry?.meridiem)}>
                        ...
                        <Row>
                            ...
                            <Col sm={12} md={7} lg={7}>
                                <Row>
                                    <Col className="mb-2">
                                        <Select options={hours} placeholder="Hour"
                                                onChange={(v) => changeOrderCancel(v, 'hour')}  value={hours.find(h=>h.value===orderCancel?.hour)}>

                                        </Select>
                                    </Col>
                                    <Col className="mb-2">
                                        <Select options={minutes} placeholder="Minute"
                                                onChange={(v) => changeOrderCancel(v, 'minute')}  value={minutes.find(h=>h.value===orderCancel?.minute)}>

                                        </Select>
                                    </Col>
                                    <Col className="mb-2">
                                        <Select options={am_pm} onChange={(v) => changeOrderCancel(v, 'meridiem')}
                                                placeholder={"AM or PM"}  value={am_pm.find(h=>h.value===orderCancel?.meridiem)}>
